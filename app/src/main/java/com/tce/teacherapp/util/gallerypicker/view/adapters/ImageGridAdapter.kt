...
import com.tce.teacherapp.util.gallerypicker.utils.DateUtil
import com.tce.teacherapp.util.gallerypicker.utils.RunOnUiThread
import com.tce.teacherapp.util.gallerypicker.utils.scroll.FastScrollRecyclerView
        ...
        doAsync {
            RunOnUiThread(ctx).safely {
                try {
                    ...
                    Glide.with(ctx).load(mimageList[holder.adapterPosition].photoUri).apply(RequestOptions().centerCrop().override(100)).transition(DrawableTransitionOptions.withCrossFade()).listener(requestListener).into(holder.image)
                } catch (e: Exception) {
                }
            }
        }
