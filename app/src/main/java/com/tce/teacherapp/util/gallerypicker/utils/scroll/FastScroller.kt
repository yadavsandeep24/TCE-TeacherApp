package com.tce.teacherapp.util.gallerypicker.utils.scroll
...
class FastScroller(context: Context, private val mRecyclerView: FastScrollRecyclerView?, attrs: AttributeSet) {
    ...
    init {
        ...
        try {
            ...
            val popupTextSize = typedArray.getDimensionPixelSize(R.styleable.FastScrollRecyclerView_fastScrollPopupTextSize,
                Utils.toScreenPixels(resources, 44f)
            )
            val popupBackgroundSize = typedArray.getDimensionPixelSize(R.styleable.FastScrollRecyclerView_fastScrollPopupBackgroundSize,
                Utils.toPixels(resources, 88f)
            )
            val popupPosition = typedArray.getInteger(R.styleable.FastScrollRecyclerView_fastScrollPopupPosition,
        ...
        mHideRunnable = Runnable {
            if (!isDragging) {
                ...
                mAutoHideAnimator = ObjectAnimator.ofInt(this@FastScroller, "offsetX", (if (Utils.isRtl(
                        mRecyclerView.resources
                    )
                ) -1 else 1) * width)
